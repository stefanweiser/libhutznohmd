...
{
    while ((lexer_state::reached_body != state_) && (-1 == get())) {
    }
    return (lexer_state::reached_body == state_);
}
...
int32_t request::get()
{
    ...
    if ((index_ >= tail_) && (false == fetch_more_data())) {
        return -1;
    }
    return static_cast<uint8_t>(raw_[index_++]);
}
