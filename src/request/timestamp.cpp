...
int32_t parse_time(int32_t& character, const http::lexer& l)
{
...
int32_t parse_month(int32_t& character, const http::lexer& l)
{
    using value_info = http::trie<int32_t>::value_info;
    ...
    static const http::trie<int32_t> t(types, -1);
...
bool parse_gmt(int32_t& character, const http::lexer& l)
{
    using value_info = http::trie<bool>::value_info;
    static const std::vector<value_info> types = {{value_info{"gmt", true}}};

    static const http::trie<bool> t(types, false);
    http::push_back_string<32> tmp;
    return t.parse(character, tmp, l);
}

void parse_optional_whitespace(int32_t& character, const http::lexer& l)
{
...
time_t parse_rfc1123_date_time(int32_t& character, const http::lexer& l)
{
    character = l.get_non_whitespace();
...
time_t parse_rfc850_date_time(int32_t& character, const http::lexer& l)
{
    parse_optional_whitespace(character, l);
    if (character != ',') {
        return -1;
...
time_t parse_asctime_date_time(int32_t& character, const http::lexer& l)
{
    parse_optional_whitespace(character, l);
    const int32_t month = parse_month(character, l);
...
std::tuple<int8_t, bool> parse_weekday(int32_t& character, const http::lexer& l)
{
    ...
    static const http::trie<value_type> t(types, value_type{-1, false});
    http::push_back_string<32> tmp;
    return t.parse(character, tmp, l);
}
