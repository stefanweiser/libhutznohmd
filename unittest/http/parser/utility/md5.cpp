...
TEST(old_md5, hello_world)
{
    std::array<uint8_t, 16> digest = calculate_md5(
        {{'H', 'e', 'l', 'l', 'o', ' ', 'W', 'o', 'r', 'l', 'd', '!'}});
    std::array<uint8_t, 16> sum{{0xED, 0x07, 0x62, 0x87, 0x53, 0x2E, 0x86, 0x36,
                                 0x5E, 0x84, 0x1E, 0x92, 0xBF, 0xC5, 0x0D,
                                 0x8C}};
    EXPECT_EQ(sum, digest);
}
...
TEST(old_md5, pangram)
{
    std::array<uint8_t, 16> digest = calculate_md5(
        {{'P', 'a', 'c', 'k', ' ', 'm', 'y', ' ', 'b', 'o', 'x', ' ', 'w', 'i',
          ...
          'l', 'i', 'q', 'u', 'o', 'r', ' ', 'j', 'u', 'g', 's'}});
    std::array<uint8_t, 16> sum{{0x1C, 0x50, 0xC8, 0x5D, 0xF8, 0xC0, 0x2C, 0x99,
                                 0xBC, 0x16, 0x05, 0xF6, 0x6E, 0x11, 0xB0,
                                 0x4A}};
    EXPECT_EQ(sum, digest);
}
